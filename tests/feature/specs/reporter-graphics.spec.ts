...
const feature = await loadFeature('tests/feature/reporter-graphics.feature')
...
describeFeature(feature, ({ Scenario, defineSteps }) => {
  ...
  defineSteps(({ Then, And }) => {
    ...
    And('the reporter HTML page contains:', (_ctx, docString) => {
      assertContains(docString ?? '')
    })
  })

  Scenario('Reporter HTML page includes visual bars for percentages', ({ Given, And, When }) => {
